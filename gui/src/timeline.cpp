...
void Timeline::OnUpdateTimer(wxTimerEvent& event) {
  // Update the display to show system time indicator.
  m_timelinePanel->Refresh();
}
...
void Timeline::ShowTimeline(bool smooth) {
  ...
  if (smooth) {
    ...
    // Immediate show for menu toggle
    m_animationProgress = 1.0;
  ...
  // Start auto-hide timer when timeline becomes visible (for both smooth and
  // immediate)
...
void Timeline::HideTimeline(bool smooth) {
  if (!m_isVisible) return;
  ...
  if (smooth) {
    ...
    // Immediate hide for menu toggle - also stop animation timer
    m_animationTimer->Stop();
    m_animationProgress = 0.0;
    ...
    if (parent && parent->IsKindOf(wxCLASSINFO(wxFrame))) {
      ...
      if (auiMgr) {
        wxAuiPaneInfo& pane = auiMgr->GetPane(this);
        if (pane.IsOk()) {
          pane.Show(false);  // Hide the pane completely for menu toggle
          auiMgr->Update();
        }
      }
    }
