...
wxDateTime Timeline::CalculateTimeIncrement() const {
  wxTimeSpan traversal_time_duration = wxTimeSpan::Seconds(60);
  double traversal_ticks = traversal_time_duration.GetSeconds().ToDouble() /
                           update_interval.GetSeconds().ToDouble();
  wxTimeSpan time_per_tick = wxTimeSpan(
      0, 0, 0, timeline_duration.GetMilliseconds().ToLong() / traversal_ticks);
...
void Timeline::OnMouseDown(wxMouseEvent& event) {
  if (m_isPlaying) OnPlayPause(wxCommandEvent());
...
void Timeline::OnMouseMove(wxMouseEvent& event) {
  if (m_isDragging) {
    ...
    double fraction =
        static_cast<double>(dx) / m_timelinePanel->GetSize().GetWidth();
...
void Timeline::OnMouseUp(wxMouseEvent& event) { m_isDragging = false; }
...
wxString Timeline::FormatDate(const wxDateTime& date,
                              const wxString& timeUnit) const {
  if (timeUnit == "hour")
